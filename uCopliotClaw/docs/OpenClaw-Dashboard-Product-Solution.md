...
本附录详细分析 OpenClaw Dashboard 可直接获取的监控与运维数据接口，涵盖智能体/多智能体监控、会话监控、通道监控、运维数据四大类别。
...
│  │  ├─ sessions.list      : 会话列表 (支持过滤/分页/搜索)             │  │
│  │  ├─ sessions.get       : 会话消息历史                              │  │
│  │  ├─ sessions.preview   : 会话预览 (最近消息摘要)                   │  │
│  │  ├─ sessions.usage     : 会话用量统计 (Token/成本/延迟)            │  │
│  │  └─ sessions.usage.timeseries : 会话用量时序数据                   │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  通道监控 (Channel Monitoring)                                    │  │
│  │  ├─ channels.status  : 通道状态 (配置/连接/探测/活动时间)          │  │
│  │  ├─ channels.logout  : 通道登出                                    │  │
│  │  └─ channel-activity : 通道活动记录 (最后收发时间)                 │  │
│  └───────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  ┌───────────────────────────────────────────────────────────────────┐  │
│  │  运维数据 (Operational Data)                                      │  │
│  │  ├─ logs.tail       : 日志流式读取 (游标/截断/滚动)                │  │
│  │  ├─ cron.list       : 定时任务列表                                 │  │
│  │  ├─ cron.status     : 定时任务状态                                 │  │
│  │  ├─ cron.runs       : 定时任务执行历史                             │  │
│  │  ├─ node.list       : 节点设备列表 (配对/连接状态)                 │  │
│  │  ├─ node.describe   : 节点详情 (能力/命令/权限)                    │  │
...
**接口路径**: `src/gateway/server-methods/health.ts` → `health` 方法

**数据模型**:

```typescript
// 来源: src/commands/health.ts
interface HealthSummary {
  ts: number;                    // 快照时间戳
  agents: AgentHealthSummary[];  // 各智能体健康状态
  channels: Record<string, ChannelHealthSummary>;  // 通道健康状态
  sessions: {
    count: number;              // 会话总数
    recent: SessionHealthEntry[];  // 最近活跃会话
  };
}

interface AgentHealthSummary {
  agentId: string;              // 智能体ID
  heartbeat: HeartbeatSummary;  // 心跳配置
  sessions: {
    count: number;              // 会话数
    recent: SessionHealthEntry[];  // 最近会话
  };
}

interface HeartbeatSummary {
  enabled: boolean;             // 是否启用
  every: string;                // 间隔描述 (如 "15m")
  everyMs: number | null;       // 间隔毫秒数
  prompt: string;               // 心跳提示词
  target: string;               // 目标通道
  model?: string;               // 使用的模型
  ackMaxChars: number;         // 确认消息最大字符数
}

interface ChannelHealthSummary {
  configured: boolean;          // 是否已配置
  accounts?: ChannelAccountSnapshot[];  // 账户快照
}
...
interface ChannelAccountSnapshot {
  ...
  ok?: boolean;                 // 探测结果
  error?: string;               // 错误信息
  lastProbeAt?: number;         // 最后探测时间
  lastInboundAt?: number;       // 最后入站消息时间
  lastOutboundAt?: number;      // 最后出站消息时间
}
```

**采集方式**:
- WebSocket RPC: `{ "method": "health", "params": { "probe": true } }`
- CLI: `openclaw health --json`
- 支持缓存 (默认 30 秒刷新间隔)
...
**接口路径**: `src/gateway/server-methods/health.ts` → `status` 方法

**数据模型**:

```typescript
// 来源: src/commands/status.types.ts
interface StatusSummary {
  runtime: string;              // 运行时版本
  link: {
    channel: string | null;     // 关联通道
    ok: boolean;                // 连接状态
  };
  heartbeat: Record<string, HeartbeatStatus>;  // 各智能体心跳状态
  channel: ChannelSummary;      // 通道摘要
  queuedSystemEvents: number;   // 队列中的系统事件
  sessions: {
    paths: string[];            // 会话存储路径
    count: number;              // 会话总数
    defaults: GatewaySessionsDefaults;  // 默认配置
    recent: SessionStatus[];    // 最近会话
    byAgent: Record<string, SessionStatus[]>;  // 按智能体分组
  };
}
...
interface SessionStatus {
  key: string;                  // 会话键
  agentId: string;              // 智能体ID
  kind: "global" | "direct" | "group" | "unknown";  // 会话类型
  ...
  model: string;                // 使用的模型
  contextTokens: number;        // 上下文 Token
  flags: {
...
interface HeartbeatStatus {
  ...
  every: string;
  everyMs: number | null;
  nextDueMs: number;            // 下次到期时间
}
```

**采集方式**:
- WebSocket RPC: `{ "method": "status" }`
- CLI: `openclaw status --json`
- 需要 `operator.admin` scope 获取敏感信息
...
#### D.2.3 agents.list - 多智能体列表

**接口路径**: `src/gateway/server-methods/agents.ts` → `agents.list` 方法

**数据模型**:

```typescript
// 来源: src/gateway/session-utils.ts
interface GatewayAgentsList {
  defaultId: string;            // 默认智能体ID
  mainKey: string;              // 主会话键
  scope: SessionScope;         // 会话作用域
  agents: GatewayAgentRow[];    // 智能体列表
}
...
interface GatewayAgentRow {
  ...
  identity?: {
    ...
    emoji?: string;             // 表情符号
    avatar?: string;            // 头像路径
    avatarUrl?: string;         // 头像 URL (Base64 或 HTTP)
  };
}
```

**采集方式**:
- WebSocket RPC: `{ "method": "agents.list" }`
...
#### D.3.1 sessions.list - 会话列表

**接口路径**: `src/gateway/server-methods/sessions.ts` → `sessions.list` 方法

**数据模型**:

```typescript
// 来源: src/gateway/session-utils.types.ts
...
interface GatewaySessionRow {
  key: string;                  // 会话键
  kind: "global" | "direct" | "group" | "unknown";  // 类型
  label?: string;               // 标签
  ...
  abortedLastRun?: boolean;     // 最后运行是否中止
  thinkingLevel?: string;       // 思考级别
  fastMode?: boolean;           // 快速模式
  verboseLevel?: number;        // 详细级别
  ...
  responseUsage?: ResponseUsage;  // 响应用量
  modelProvider?: string;       // 模型提供商
  model: string;                // 模型名称
  contextTokens?: number;       // 上下文 Token
  deliveryContext?: string;     // 投递上下文
  lastChannel?: string;         // 最后通道
  lastTo?: string;              // 最后接收者
  lastAccountId?: string;       // 最后账户ID
  derivedTitle?: string;        // 派生标题
...
interface SessionsListParams {
  ...
  includeDerivedTitles?: boolean;  // 包含派生标题
  includeLastMessage?: boolean; // 包含最后消息预览
  spawnedBy?: string;           // 按创建者过滤
  label?: string;               // 按标签过滤
  agentId?: string;             // 按智能体过滤
  search?: string;              // 搜索关键词
...
**采集方式**:
- WebSocket RPC: `{ "method": "sessions.list", "params": { "limit": 50, "activeMinutes": 60 } }`
...
#### D.3.2 sessions.usage - 会话用量统计

**接口路径**: `src/gateway/server-methods/usage.ts` → `sessions.usage` 方法

**数据模型**:

```typescript
// 来源: src/gateway/server-methods/usage.ts
interface SessionsUsageResult {
  updatedAt: number;            // 更新时间
  startDate: string;            // 开始日期 (YYYY-MM-DD)
  endDate: string;              // 结束日期 (YYYY-MM-DD)
  sessions: SessionUsageEntry[];  // 会话用量条目
  totals: CostUsageTotals;      // 总计
  aggregates: SessionsUsageAggregates;  // 聚合数据
}
...
interface SessionUsageEntry {
  ...
  channel?: string;             // 通道
  chatType?: string;            // 聊天类型
  origin?: SessionOrigin;       // 来源
  modelOverride?: string;       // 模型覆盖
  providerOverride?: string;    // 提供商覆盖
  modelProvider?: string;       // 模型提供商
  model?: string;               // 模型
  usage?: SessionCostUsage;     // 用量详情
  contextWeight?: SystemPromptReport | null;  // 上下文权重
}
...
interface CostUsageTotals {
  ...
  cacheReadCost: number;        // 缓存读取成本
  cacheWriteCost: number;       // 缓存写入成本
  missingCostEntries: number;   // 缺失成本条目数
}

interface SessionsUsageAggregates {
  messages: SessionMessageCounts;  // 消息统计
  tools: {
    totalCalls: number;         // 工具调用总数
    uniqueTools: number;        // 唯一工具数
    tools: Array<{ name: string; count: number }>;  // 工具使用排行
  };
  byModel: SessionModelUsage[];  // 按模型聚合
  byProvider: SessionModelUsage[];  // 按提供商聚合
  byAgent: Array<{ agentId: string; totals: CostUsageTotals }>;  // 按智能体聚合
  byChannel: Array<{ channel: string; totals: CostUsageTotals }>;  // 按通道聚合
  byDate: Array<{ date: string; tokens: number; cost: number; messages: number }>;  // 按日期聚合
  latency?: {
    count: number;              // 请求计数
    avg: number;                // 平均延迟 (ms)
    min: number;                // 最小延迟
  ...
  dailyLatency?: Array<{ date: string; count: number; avg: number }>;  // 每日延迟
}
```
...
```typescript
interface SessionsUsageParams {
  startDate?: string;           // 开始日期 (YYYY-MM-DD)
  endDate?: string;             // 结束日期 (YYYY-MM-DD)
  days?: number;                // 天数 (默认 30)
  mode?: "utc" | "gateway" | "specific";  // 日期模式
...
**采集方式**:
- WebSocket RPC: `{ "method": "sessions.usage", "params": { "days": 7 } }`
- 支持缓存 (30 秒 TTL)
...
#### D.3.3 sessions.usage.timeseries - 用量时序数据

**接口路径**: `src/gateway/server-methods/usage.ts` → `sessions.usage.timeseries` 方法

**数据模型**:

```typescript
interface SessionUsageTimeseries {
  sessionId: string;
  points: Array<{
    ts: number;                 // 时间戳
    input: number;              // 输入 Token
...
**采集方式**:
- WebSocket RPC: `{ "method": "sessions.usage.timeseries", "params": { "key": "agent:ops:main" } }`
...
#### D.4.1 channels.status - 通道状态

**接口路径**: `src/gateway/server-methods/channels.ts` → `channels.status` 方法

**数据模型**:

```typescript
// 来源: src/gateway/server-methods/channels.ts
interface ChannelsStatusResult {
  ts: number;                   // 查询时间戳
  channelOrder: string[];       // 通道顺序
  channelLabels: Record<string, string>;  // 通道标签
  channelDetailLabels: Record<string, string>;  // 详细标签
  channelSystemImages: Record<string, string>;  // 系统图标
  channelMeta: Record<string, ChannelMetaEntry>;  // 通道元数据
  channels: Record<string, ChannelSummary>;  // 通道摘要
  channelAccounts: Record<string, ChannelAccountSnapshot[]>;  // 账户快照
  channelDefaultAccountId: Record<string, string>;  // 默认账户ID
}
...
interface ChannelsStatusParams {
  probe?: boolean;              // 是否探测 (默认 false)
  timeoutMs?: number;           // 探测超时 (默认 10000)
}
```

**采集方式**:
- WebSocket RPC: `{ "method": "channels.status", "params": { "probe": true } }`
...
#### D.4.2 channel-activity - 通道活动记录

**接口路径**: `src/infra/channel-activity.ts`
...
interface ActivityEntry {
  inboundAt: number | null;     // 最后入站消息时间
  outboundAt: number | null;    // 最后出站消息时间
}
...
**采集方式**:
- 通过 `channels.status` 接口间接获取
- 内部函数: `getChannelActivity({ channel, accountId })`
...
#### D.5.1 logs.tail - 日志流式读取

**接口路径**: `src/gateway/server-methods/logs.ts` → `logs.tail` 方法
...
interface LogsTailResult {
  file: string;                 // 日志文件路径
  cursor: number;               // 当前游标 (文件大小)
  size: number;                 // 文件大小
  lines: string[];              // 日志行
  truncated: boolean;           // 是否截断
  reset: boolean;               // 是否重置游标
}
```
...
interface LogsTailParams {
  cursor?: number;              // 游标位置 (用于增量读取)
  limit?: number;               // 行数限制 (默认 500, 最大 5000)
  maxBytes?: number;            // 最大字节数 (默认 250KB, 最大 1MB)
}
```

**采集方式**:
- WebSocket RPC: `{ "method": "logs.tail", "params": { "cursor": 12345 } }`
- 支持滚动日志文件自动发现 (`openclaw-YYYY-MM-DD.log`)
...
**接口路径**: `src/gateway/server-methods/cron.ts`
...
interface CronJob {
  id: string;                   // 任务ID
  name: string;                 // 任务名称
  schedule: string;             // 调度表达式 (cron/interval)
  enabled: boolean;             // 是否启用
...
interface CronStatus {
  ...
  jobsCount: number;            // 任务数量
  nextRunAtMs?: number;         // 下次运行时间
}

// cron.runs - 执行历史
interface CronRunLogPage {
...
interface CronRunLogEntry {
  ...
  jobId: string;                // 任务ID
  jobName?: string;             // 任务名称
  scheduledAtMs: number;        // 调度时间
  startedAtMs: number;          // 开始时间
  finishedAtMs?: number;        // 完成时间
  status: "ok" | "error" | "skipped";  // 状态
  error?: string;               // 错误信息
...
- WebSocket RPC:
  - `{ "method": "cron.list" }`
  - `{ "method": "cron.status" }`
  - `{ "method": "cron.runs", "params": { "scope": "all", "limit": 50 } }`
...
**接口路径**: `src/gateway/server-methods/nodes.ts`
...
// node.describe - 节点详情
interface NodeDescribeResult extends NodeEntry {
  // 同 NodeEntry，但针对单个节点
}
```
...
- WebSocket RPC:
  - `{ "method": "node.list" }`
  - `{ "method": "node.describe", "params": { "nodeId": "xxx" } }`
...
#### D.5.4 usage.status - Provider 用量摘要

**接口路径**: `src/gateway/server-methods/usage.ts` → `usage.status` 方法

**数据模型**:

```typescript
// 来源: src/infra/provider-usage.ts
...
**采集方式**:
- WebSocket RPC: `{ "method": "usage.status" }`
...
│       │                                                     │
│       └─► 推送事件 (broadcast)                              │
│           ├─► node.pair.requested  → 设备配对请求            │
│           ├─► node.pair.resolved   → 设备配对结果            │
│           └─► ...                                          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Dashboard Backend (定时任务)                                │
│       │                                                     │
│       ├─► sessions.usage (每小时)  → 用量统计聚合            │
│       │    └─► 存储到时序数据库 (InfluxDB/Prometheus)        │
│       │                                                     │
│       ├─► cron.runs (每分钟)      → 任务执行历史             │
│       │    └─► 存储到关系数据库 (SQLite/PostgreSQL)          │
│       │                                                     │
│       └─► usage.status (每 5 分钟) → Provider 用量          │
│            └─► 存储到时序数据库                              │
│                                                             │
│  数据保留策略:                                               │
│  - 实时数据: 24 小时                                        │
...
| 日志查看 | logs.tail | 实时流 | 日志流、搜索过滤、错误追踪 |
| 定时任务 | cron.list, cron.runs | 60s | 任务列表、执行历史、状态监控 |
| 用量统计 | sessions.usage, usage.status | 5min | 成本报告、模型分布、延迟分析 |

### D.7 接口调用示例

#### D.7.1 WebSocket RPC 调用
...
# 会话列表
openclaw sessions list --json --limit 50

# 设备列表
openclaw node list --json
```
...
for (const agent of agents) {
  await rpc('sessions.list', { agentId: agent.id }); // ❌ N+1 问题
}
```
...
async function syncLogs() {
  const result = await rpc('logs.tail', { cursor });
  if (result.lines.length > 0) {
    await storeLogs(result.lines);
...
async function syncSessions() {
  const result = await rpc('sessions.list', {
    activeMinutes: 60,  // 只同步活跃会话
    limit: 100
  });
  const updated = result.sessions.filter(s => s.updatedAt > lastUpdatedAt);
  await storeSessions(updated);
  lastUpdatedAt = Math.max(...updated.map(s => s.updatedAt));
}
```
